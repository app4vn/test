...
let isFilteringByNotesOnly = false;
...
// --- Logic cho Sidebar Desktop (Thu gọn/Mở rộng) ---
function handleDesktopSidebarView() {
    const isDesktop = window.innerWidth >= 769;
    if (isDesktop) {
        document.body.classList.add('desktop-view'); // Kích hoạt CSS cho sidebar desktop
        // CSS sẽ tự xử lý việc thu gọn/mở rộng bằng :hover
        // Đảm bảo không có class của mobile sidebar đang active
        if (document.body.classList.contains('sidebar-open')) {
            closeMobileSidebar();
        }
    } else {
        document.body.classList.remove('desktop-view');
        // Logic mobile sidebar sẽ hoạt động bình thường
    }
}
...
function openMobileSidebar() {
    if (document.body.classList.contains('desktop-view')) return; // Không mở mobile sidebar trên desktop
...
if (mobileMenuBtn) {
    mobileMenuBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        if (document.body.classList.contains('desktop-view')) return; // Không dùng nút này trên desktop
...
onAuthStateChanged(auth, (user) => {
    if (user) {
        ...
        closeAuthModal();
        closeMobileSidebar(); // Đảm bảo mobile sidebar đóng khi đăng nhập
        if (searchInput) searchInput.disabled = false;
        if (sortSelect) sortSelect.disabled = false;
        ...
        handleDesktopSidebarView(); // Kích hoạt logic sidebar desktop
    } else {
        console.log("User logged out.");
        currentUser = null;
        document.body.classList.remove('logged-in');
        document.body.classList.add('logged-out');
        document.body.classList.remove('desktop-view'); // Xóa class desktop khi logout
        closeAuthModal();
        closeMobileSidebar();
        clearEditor();
...
function renderNotesList(notesFromCache) {
    ...
    let notesToRender = notesFromCache.filter(note => {
        if (isFilteringByTodo) {
            if (!note.todos || note.todos.length === 0) return false;
    ...
    let currentTitleText = "Tất cả";
    if (isFilteringByTodo) currentTitleText = "Công việc";
    else if (isFilteringByNotesOnly) currentTitleText = "Chỉ Ghi chú";
    ...
    if (notesToRender.length === 0) {
        ...
        if(activeTag || currentSearchTerm) {
            message = `Không tìm thấy mục nào khớp với bộ lọc hiện tại.`;
        }
    ...
    if (mainViewTitle) mainViewTitle.textContent = currentTitleText;
...
function renderTagsList(notes) {
    ...
    allTagElement.addEventListener('click', () => {
        if (activeTag !== null || isFilteringByTodo || isFilteringByNotesOnly) {
            activeTag = null;
            // Các bộ lọc isFilteringByTodo và isFilteringByNotesOnly sẽ được reset trong showMainNotesView
            showMainNotesView();
        }
    });
...
// --- EVENT LISTENERS CHO CÁC NÚT LỌC LOẠI GHI CHÚ ---
if (filterTodoNotesBtn) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    toggleTodoEditorVisibility();
    handleDesktopSidebarView(); // Kiểm tra và áp dụng trạng thái sidebar desktop ban đầu
    if (!auth.currentUser) {
         document.body.classList.remove('logged-in');
         document.body.classList.add('logged-out');
    }
...
window.addEventListener('resize', handleDesktopSidebarView); // Cập nhật trạng thái sidebar khi resize
